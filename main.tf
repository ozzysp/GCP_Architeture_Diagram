provider "google" {
  project = "OzzyScubaOrg"
  region  = "us-central1"
  zone    = "us-central1-c"
  credentials = "${file("serviceaccount.yaml")}"
}

resource "google_folder" "Financial" {
  display_name = "Financial"
  parent       = "organizations/540829645030"
}

resource "google_folder" "SalesForce" {
  display_name = "SalesForce"
  parent       = google_folder.Financeiro.name
}

resource "google_folder" "Development" {
  display_name = "Development"
  parent       = google_folder.SalesForce.name
}

resource "google_folder" "Deploy" {
  display_name = "Deploy"
  parent       = google_folder.SalesForce.name
}


resource "google_project" "OzzyScubaOrg-salesforce-dev" {
  name       = "SalesForce-Dev"
  project_id = "OzzyScubaOrg-salesforce-dev"
  folder_id  = google_folder.Deploy.name
  auto_create_network=false
  billing_account = "018973-A8340F-83D8E5"

}

# resource "google_project" "OzzyScubaOrg-salesforce-prod" {
#   name       = "SalesForce-Prod"
#   project_id = "OzzyScubaOrg-salesforce-prod"
#   folder_id  = google_folder.Deploy.name
#   auto_create_network=false
#   billing_account = "01B2C4-70459E-F53446"
# }